...
**Delegation Contract（调度声明，MANDATORY）**：

每次 Spawn 子代理**之前**，Team Lead 必须输出标准化声明，使调度可追溯、可审计：
...
📋 Delegation Contract:
  ...
  READ: [{需读取的文件列表}]
  WRITE: [{允许写入的文件/目录范围}]
  CONSTRAINTS: [{约束列表: TDD流程/编码规范/文件大小限制等}]
  VERIFY: [{完成后的验证命令和预期结果}]
```

> 灵感来源: atelier-pipeline 的 Delegation Contract 机制 —— 静默调度（不声明就 Spawn）
> 与"不声明就提交"同级别违规。
...
📋 Delegation Contract:
  ...
  READ: [CLAUDE.md, docs/plans/task-001.md, docs/plans/task-002.md, docs/plans/task-003.md]
  WRITE: [src/, unit_tests/, API_tests/, run_tests.sh, run_tests.bat, .dockerignore, Dockerfile, docker-compose.yml]
  CONSTRAINTS: [TDD Red-Green-Refactor, API端点真实调用(禁止全栈Mock), 强制本地运行测试]
  VERIFY: [bash run_tests.sh, exit code 0]

你是 Implementer-Backend。
1. 读取 CLAUDE.md 了解项目规范。
2. 依次执行 task-001, task-002, task-003。
3. 遵循 TDD：先写失败测试(RED) → 写实现(GREEN) → 重构。
4. 你只能编辑: src/, unit_tests/, API_tests/ 以及根目录下的测试/构建脚本。
5. 完成每个任务后运行验证步骤并报告结果。完成所有任务前强制自己执行一次 `./run_tests.sh`！
```
...
- 所有 Implementer/Reviewer 子代理必须 `wait` 到最终状态后，才允许更新 `docs/plans/_index.md` 为 `done`。
- 在 `docs/plans/_index.md` 仍存在 `pending` 或 `in_progress` 时，**禁止**输出 `<promise>EXECUTION_COMPLETE</promise>`。

### 🚀 强制测试执行验证 (Pre-Flight Execution Validation)

**在返回 `EXECUTION_COMPLETE` 之前，必须执行以下硬门禁操作**：
1. **真实调用原则**：`API_tests/` 下的测试用例**绝对禁止使用纯 Mock 框架**来伪造路由或响应。必须通过启动包含真实路由框架的对象（例如通过 FastAPI TestClient / Supertest 或发真实 HTTP 请求）来进行黑盒调用。对于需要外部网络的第三方请求可以适当 Mock，但业务链路自身必须全真。
2. **强制运行自测**：在生成完毕功能代码和测试脚本后，智能体**必须在自己的系统能力范围内执行 `./run_tests.sh` 或等效测试命令**。
3. **消除幻觉**：如果运行抛出错误或语法异常，必须在当前迭代中当场修复它；**绝不能看都不看或带着报错假装没看见**就提交 `EXECUTION_COMPLETE`。
