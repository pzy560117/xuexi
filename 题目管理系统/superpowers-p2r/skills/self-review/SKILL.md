...
### Step 3: 执行验收审查（要求使用多子代理并行审查）

**强制要求**：放弃单线程 Inline 模式，必须采用并行任务执行 (Parallel Task Execution) 拉起多个专业子代理（执行前须分别读取 `skills/code-reviewer/SKILL.md`、`skills/refactor-cleaner/SKILL.md`、`skills/e2e-runner/SKILL.md`）：
- **`code-reviewer`**：接管代码质量、工程架构及测试覆盖度审计。
- **`refactor-cleaner`**：识别并剔除实现过程中的废弃死代码(Dead code)和冗余。
- **`e2e-runner`**（如有对应测试）：执行大范围黑盒/浏览器端对端验证。
...
### Step 4: 安全专项审查（要求使用子代理）

**强制要求**：派发给专业的 `security-reviewer` 子代理执行专项安全审计。必须先读取 `skills/security-reviewer/SKILL.md`。
