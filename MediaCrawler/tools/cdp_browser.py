...
class CDPBrowserManager:
    ...
    async def launch_and_connect(
        ...
        优先复用已运行在 CDP_DEBUG_PORT 上的浏览器实例（保持 session），
        如果没有已运行的浏览器则启动新实例。
        """
        try:
            # 1. 尝试复用已运行的浏览器（优先检测默认 CDP 端口）
            default_port = config.CDP_DEBUG_PORT
            if await self._try_reuse_existing_browser(default_port, playwright):
                utils.logger.info(
                    f"[CDPBrowserManager] Reusing existing browser on port {default_port}"
                )
                self.debug_port = default_port

                # 创建/获取 browser context
            ...
            # Get available port (使用默认端口，避免端口偏移)
    ...
    async def _try_reuse_existing_browser(
        self, debug_port: int, playwright: Playwright
    ) -> bool:
        """
        尝试复用已运行在指定端口上的浏览器实例。

        通过连接 CDP WebSocket 来检测浏览器是否存在且可用。
        ...
        try:
            ...
            utils.logger.info(
                f"[CDPBrowserManager] Detected existing browser on port {debug_port}, trying to reuse..."
            )

            # 获取 WebSocket URL 并尝试连接
            ...
            if self.browser.is_connected():
                utils.logger.info(
                    f"[CDPBrowserManager] Successfully reused existing browser, "
                    f"contexts: {len(self.browser.contexts)}"
                )
                return True
            else:
                utils.logger.warning(
                    "[CDPBrowserManager] Connected but browser is not in ready state"
                )
                return False

        except Exception as e:
            utils.logger.info(
                f"[CDPBrowserManager] Cannot reuse existing browser: {e}"
            )
            return False
    ...
    async def _launch_browser(self, browser_path: str, headless: bool):
        ...
        user_data_dir 选取优先级：
        1. config.CDP_USER_DATA_DIR（用户个人 Chrome Profile，复用登录态）
        2. config.SAVE_LOGIN_STATE 时自动创建独立目录
        3. 无 user_data_dir（全新临时 profile）
        """
        user_data_dir = None

        # 优先使用用户指定的 Chrome Profile 目录
        custom_dir = getattr(config, "CDP_USER_DATA_DIR", "")
        if custom_dir and os.path.isdir(custom_dir):
            user_data_dir = custom_dir
            utils.logger.info(
                f"[CDPBrowserManager] Using personal Chrome profile: {user_data_dir}"
            )
        elif config.SAVE_LOGIN_STATE:
            # 自动创建独立的 CDP Profile 目录
