...
class BilibiliCrawler(AbstractCrawler):
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            elif config.CRAWLER_TYPE == "report":
    ...
    async def _report_process_video(
        self,
        ...
        """
        处理单个视频的详情获取、字幕提取、评论爬取并添加到报告。

        report 模式的核心处理逻辑，被关键词搜索和指定视频两种入口共用。

        Args:
            report: 报告生成器实例
            aid: 视频 aid
            bvid: 视频 BV 号
            title: 搜索结果中的标题（可被详情覆盖）
            desc: 搜索结果中的描述（可被详情覆盖）

        Returns:
            True 表示处理成功，False 表示跳过
        """
        try:
            # 1. 获取视频详情（含 cid）
            detail = await self.bili_client.get_video_info(aid=aid, bvid=bvid)
            await asyncio.sleep(config.CRAWLER_MAX_SLEEP_SEC)

            view_data = detail.get("View", {})
            cid = view_data.get("cid", 0)
            owner = view_data.get("owner", {})
            stat = view_data.get("stat", {})

            # 使用详情中的完整信息
            title = view_data.get("title", title)
            desc = view_data.get("desc", desc)
            aid = view_data.get("aid", aid)  # 详情中 aid 更可靠
            bvid = view_data.get("bvid", bvid)
            nickname = owner.get("name", "")
            liked_count = stat.get("like", 0)
            comment_count = stat.get("reply", 0)
            share_count = stat.get("share", 0)
            pub_date = view_data.get("pubdate", 0)
            create_time = ""
            if pub_date:
                try:
                    create_time = datetime.fromtimestamp(pub_date).strftime("%Y-%m-%d %H:%M")
                except (OSError, ValueError):
                    create_time = str(pub_date)

            url = f"https://www.bilibili.com/video/{bvid}" if bvid else f"https://www.bilibili.com/video/av{aid}"

            # 2. 获取字幕（字幕 API 优先，毫秒级）
            transcript = ""
            if cid:
                transcript = await self.bili_client.get_video_subtitle(bvid=bvid or str(aid), cid=cid)
            ...
            # 4. 获取评论（report 模式获取全部评论，不受 CRAWLER_MAX_COMMENTS_COUNT_SINGLENOTES 限制）
            if config.ENABLE_GET_COMMENTS:
                try:
                    utils.logger.info(f"[_report_process_video] Fetching all comments for aid={aid}")
                    comments = await self.bili_client.get_video_all_comments(
                        video_id=str(aid),
                        crawl_interval=config.CRAWLER_MAX_SLEEP_SEC,
                        is_fetch_sub_comments=False,
                        max_count=9999999,  # report 模式不限量，靠 is_end 自然终止
                    )
                    formatted_comments = []
                    for c in comments:
                        member = c.get("member", {})
                        content_info = c.get("content", {})
                        formatted_comments.append({
                            "nickname": member.get("uname", ""),
                            "content": content_info.get("message", ""),
                            "like_count": c.get("like", 0),
                            "create_time": c.get("ctime", 0),
                        })
                    report.add_comments(str(aid), formatted_comments)
                    utils.logger.info(f"[_report_process_video] Got {len(formatted_comments)} comments for aid={aid}")
                except Exception as e:
                    utils.logger.warning(
                        f"[BilibiliCrawler._report_process_video] Failed to get comments for {aid}: {e}"
                    )

            utils.logger.info(
                f"[BilibiliCrawler._report_process_video] Processed: {title[:40]}"
            )
            return True

        except Exception as e:
            utils.logger.error(
                f"[BilibiliCrawler._report_process_video] Error processing video aid={aid} bvid={bvid}: {e}"
            )
            return False

    async def generate_report(self):
        """
        生成内容报告（report 模式核心方法）

        支持两种入口：
        1. 指定视频模式：当 BILI_SPECIFIED_ID_LIST 非空时，直接获取指定视频详情
        2. 关键词搜索模式：按关键词搜索视频列表

        流程：获取视频详情 → 字幕API提取口播文案 → 爬取评论 → 生成Markdown
        """
        utils.logger.info("[BilibiliCrawler.generate_report] Begin generate bilibili report")

        # 模式一：指定视频 URL/ID 直接生成报告
        specified_ids = getattr(config, "BILI_SPECIFIED_ID_LIST", [])
        if specified_ids:
            await self._generate_report_for_specified_videos(specified_ids)
            return

        # 模式二：关键词搜索模式
        await self._generate_report_by_keywords()

    async def _generate_report_for_specified_videos(self, video_url_list: List[str]):
        """
        根据指定的视频 URL/ID 列表生成报告

        Args:
            video_url_list: 视频 URL 或 BV 号列表
        """
        utils.logger.info(
            f"[BilibiliCrawler.generate_report] Specified video mode, {len(video_url_list)} video(s)"
        )
        ...
        for video_url in video_url_list:
            try:
                video_info = parse_video_info_from_url(video_url)
                bvid_list.append(video_info.video_id)
                utils.logger.info(
                    f"[BilibiliCrawler.generate_report] Parsed video ID: {video_info.video_id}"
                )
            except ValueError as e:
                utils.logger.error(
                    f"[BilibiliCrawler.generate_report] Failed to parse video URL: {e}"
                )
                continue

        if not bvid_list:
            utils.logger.warning("[BilibiliCrawler.generate_report] No valid video IDs found")
            return

        # 用第一个 bvid 作为报告关键词标识
        keyword = bvid_list[0] if len(bvid_list) == 1 else f"指定视频x{len(bvid_list)}"
        report = ReportGenerator(keyword=keyword, platform="bili")

        # 逐个处理
        for bvid in bvid_list:
            await self._report_process_video(report=report, aid=0, bvid=bvid)
            await asyncio.sleep(config.CRAWLER_MAX_SLEEP_SEC)

        # 生成报告文件
        filepath = await report.generate()
        utils.logger.info(f"[BilibiliCrawler.generate_report] Report saved: {filepath}")

    async def _generate_report_by_keywords(self):
        """
        根据关键词搜索视频并生成报告（原始模式）
        """
        bili_limit_count = 20
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            source_keyword_var.set(keyword)
            report = ReportGenerator(keyword=keyword, platform="bili")
            utils.logger.info(f"[BilibiliCrawler.generate_report] Processing keyword: {keyword}")
            ...
            while collected < max_notes:
                try:
                    videos_res = await self.bili_client.search_video_by_keyword(
                        keyword=keyword, page=page, page_size=bili_limit_count,
                    )
                    video_list: List[Dict] = videos_res.get("result", [])
                    if not video_list:
                        utils.logger.info(
                            f"[BilibiliCrawler.generate_report] No more results for '{keyword}' at page {page}"
                        )
                        break
                except Exception as e:
                    utils.logger.error(f"[BilibiliCrawler.generate_report] Search failed: {e}")
                    break
                ...
                for video_item in video_list:
                    ...
                    desc = video_item.get("description", "")

                    success = await self._report_process_video(
                        report=report, aid=aid, bvid=bvid, title=title, desc=desc,
                    )
                    if success:
                        collected += 1
                        utils.logger.info(
                            f"[BilibiliCrawler.generate_report] Collected {collected}/{max_notes}"
                        )
