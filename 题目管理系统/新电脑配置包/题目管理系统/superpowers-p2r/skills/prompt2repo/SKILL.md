...
| `--skip-mutation-test` |  ❌  | 跳过 Phase 3.55 变异测试门禁   |
...
### 9. Phase 3.55★ 质量门禁（变异测试）

```
如未指定 --skip-mutation-test：
执行变异测试门禁：
1. 根据项目语言选择变异测试工具（Python: mutmut / JS/TS: stryker）
2. 对核心业务代码执行变异测试
3. 变异存活率阈值 ≤ 30%（即 ≥ 70% 的变异体被测试击杀）
4. 识别“虚假通过”的测试（变异体未被击杀 = 测试未有效验证代码行为）
5. 输出报告到 .tmp/mutation-test-report.md
6. 若超过阈值，自动补充缺失的测试用例并重新验证
等待 `<promise>MUTATION_TEST_COMPLETE</promise>` 信号。
```
