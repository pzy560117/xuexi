...
- `FINAL_VERDICT: PASS`
- `TASK-*` 包测试目录真实存在（优先 `repo/tests/unit_tests` + `repo/tests/API_tests`，兼容 `repo/unit_tests` + `repo/API_tests`）
...
检查：
- `TASK-*/repo/tests/unit_tests` 与 `TASK-*/repo/tests/API_tests`（优先结构）
- 或兼容旧结构：`TASK-*/repo/unit_tests` 与 `TASK-*/repo/API_tests`
...
- REPO_TEST_LAYOUT: <nested|legacy-root|missing>
## Report Consistency
- R1_MAIN: <PASS|FAIL>
...
- `TASK-*` 测试目录结构满足（nested 优先，legacy-root 兼容）
