...
- Chosen approach is finalized with documented rationale
...
1. **Converge in Order**: Clarify → Compare → Choose (auto by default) → Design → Reflect → Commit → Transition
...
Explore codebase first, then resolve requirements autonomously. Ask the user only when blocked by irreversible business ambiguity.
...
4. **Resolve gaps automatically** - Infer defaults from prompt + codebase + project standards
   - Prefer deterministic, least-surprise defaults
   - Record assumptions explicitly in design docs and/or `questions.md`
   - Use AskUserQuestion only if a decision is legally/business-critical and cannot be inferred safely
...
Research existing patterns, propose viable options, and converge on the best option automatically.
...
4. **Auto select** - Choose the recommended option directly and continue
   - Document trade-offs and why alternatives were not selected
   - Use AskUserQuestion only when explicit stakeholder choice is mandatory
...
**Output**: Finalized approach with rationale and trade-offs understood.
...
5. **Proceed autonomously**: Present reflection summary and continue to commit without waiting for manual selection

**Output**: Updated design documents with issues resolved and assumptions documented.
