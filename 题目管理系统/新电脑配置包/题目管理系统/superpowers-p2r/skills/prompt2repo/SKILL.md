---
name: prompt2repo
description: "Prompt2Repo 一键入口: 从 prompt.md 到完整交付包的全自动流水线，串联 Phase 0-4.5 + 4 个质量门禁"
---
...
2. **主动管理阶段状态 (State Advancement Duty) ★ 绝对不可省略**：
   Loop 脚本无法自动猜测进度！当你准备结束当前阶段（即：准备输出该阶段的信号词，如 `PROMPT_PARSING_COMPLETE` 等）时，你**必须**先使用文件写入工具（Write/Replace）主动修改 `.claude/superpower-loop.local.md` 文件：
