...
**CRITICAL**: 输出 `<promise>SELF_REVIEW_COMPLETE</promise>` 时必须遵守：
- 6 维度验收审查全部完成
- 安全专项审查完成
- 测试覆盖度静态审计完成
...
**核心概念**: 自测审查是“交付前的良心检查”，确保项目满足 QuestionSystem 官方验收标准。

- **MANDATORY**: 必须使用多镜头 Reviewer 并行审查（Security/Quality/UX）
- **MANDATORY**: 每条审查必须给出结论 + 证据(file:line) + 可复现验证方式
- **MANDATORY**: 阻塞级/高级问题必须自动修复
- **PROHIBITED**: 不得跳过任何维度的审查
- **PROHIBITED**: 不得在没有证据的情况下标记 Pass

## 概述

本 Skill 是 Prompt2Repo 流水线的自测阶段。在 Phase 2 完成项目生成后，使用 QuestionSystem 官方验收提示词对项目进行全面审查。
...
### Step 3: 执行验收审查（多镜头 Reviewer 并行模式，失败自动降级）

优先采用 **Agent Team 多镜头并行审查**（参考 `skills/agent-team-driven-development/SKILL.md`），spawn 4 个专业 Reviewer 子代理（含盲审，实现信息不对称 Review）：

#### 🔍 Reviewer 并行分派

| Reviewer 角色 | 聚焦维度 | Spawn 上下文 |
|:---|:---|:---|
| **Reviewer-Security** | 安全漏洞 + 输入校验 + 密钥泄露 + 鉴权 | `skills/security-reviewer/SKILL.md` + 安全检查清单 |
| **Reviewer-Quality** | 代码质量 + 架构合规 + 测试覆盖度 + 工程细节 | `skills/code-reviewer/SKILL.md` + `skills/refactor-cleaner/SKILL.md` |
| **Reviewer-UX** | 可运行性 + 需求适配度 + 美观度（前端场景）| `skills/e2e-runner/SKILL.md` + prompt.md |
| **Reviewer-Blind** | 纯 diff 盲审：仅基于 git diff 的技术审查，无需求上下文 | `skills/blind-reviewer/SKILL.md`（信息不对称，防止锚定效应）|

**Reviewer 子代理 Spawn 示例**：
```
你是 Reviewer-Security。
1. 读取 skills/security-reviewer/SKILL.md 了解安全审计标准。
2. 逐条检查: 认证入口、路由鉴权、对象级授权、密钥暴露、SQL注入等。
3. 每条检查必须给出: 结论(Pass/Fail) + 证据(file:line) + 可复现验证方式。
4. 输出结果到 .tmp/review-security.md。
```

**结果汇总**：Team Lead（主代理）收集 4 个 Reviewer 的审查结果后：
1. 合并去重：同一问题被多个 Reviewer 发现时，保留最严重级别
2. 冲突仲裁：若 Reviewer 间结论矛盾，以更严格的结论为准
3. **盲审权重加成**：Reviewer-Blind 发现的问题权重 +1（因为无上下文锚定，更客观）
4. 统一输出：合并为 `.tmp/self-review-report.md`
...
输出 `<promise>SELF_REVIEW_COMPLETE</promise>`，且该标签必须是回复最后一行。

## References

- `./references/ai-report-prompt-zh.md` - 中文验收提示词模板
- `./references/ai-report-prompt-en.md` - 英文验收提示词模板
- `../security-reviewer/SKILL.md` - 安全审计子代理
- `../code-reviewer/SKILL.md` - 代码质量审查子代理
- `../e2e-runner/SKILL.md` - E2E 验证子代理
