...
**CRITICAL**: 输出 `<promise>CHECKLIST_COMPLETE</promise>` 时必须遵守：
- 所有适用的检查清单已生成并执行
- 可自动修复的项已修复
- 未修复项已记录
...
**核心概念**: 领域检查清单是“按行业标准检查”，确保代码不仅功能正确，还符合安全/质量/合规的行业底线。

- **MANDATORY**: 必须拉起 `security-reviewer` 子代理主导安全检查
- **MANDATORY**: 每个检查项必须通过文件内容和目录结构静态扫描验证
- **MANDATORY**: 未通过项必须附原因和修复建议
- **PROHIBITED**: 不得跳过安全检查清单
- **PROHIBITED**: 不得在有 FAIL 级检查项时输出 Promise
...
输出 `<promise>CHECKLIST_COMPLETE</promise>`，且该标签必须是回复最后一行。
...
- `../speckit-checklist/SKILL.md` - speckit-checklist 方法论来源
