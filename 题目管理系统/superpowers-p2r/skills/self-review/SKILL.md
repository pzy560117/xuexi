...
**核心概念**: 自测审查是“交付前的良心检查”，确保项目满足题目管理系统官方验收标准。
...
优先采用 **Agent Team 多镜头并行审查**（参考 `skills/agent-team-driven-development/SKILL.md`），spawn 3 个专业 Reviewer 子代理：
...
**结果汇总**：Team Lead（主代理）收集 3 个 Reviewer 的审查结果后：
1. 合并去重：同一问题被多个 Reviewer 发现时，保留最严重级别
2. 冲突仲裁：若 Reviewer 间结论矛盾，以更严格的结论为准
3. 统一输出：合并为 `.tmp/self-review-report.md`
