...
class BilibiliClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def get_video_all_comments(
        ...
        while not is_end and len(result) < max_count:
            ...
            if not isinstance(is_end, bool):
                utils.logger.warning(f"[BilibiliClient.get_video_all_comments] 'is_end' is not a boolean (value={is_end}, type={type(is_end)}) for video_id: {video_id}. Assuming end of comments.")
                is_end = True
    ...
    async def get_video_subtitle(self, bvid: str, cid: int) -> str:
        """
        获取 B 站视频的 AI 自动字幕文本

        通过 /x/player/wbi/v2 接口获取字幕列表，优先选择 AI 中文字幕，
        下载字幕 JSON 并拼接为纯文本。
        ...
        try:
            ...
            resp = await self.get(uri, params, enable_params_sign=True)

            subtitles = resp.get("subtitle", {}).get("subtitles", [])
            if not subtitles:
                utils.logger.info(
                    f"[BilibiliClient.get_video_subtitle] No CC subtitles for bvid={bvid}, cid={cid}, "
                    f"trying AI Summary API fallback..."
                )
                # 降级：尝试 AI 总结 API 获取转录文本
                return await self.get_video_ai_summary(bvid=bvid, cid=cid)

            # 2. 优先选择 AI 中文字幕，其次任何中文字幕
            target = None
            ...
            subtitle_data = await self._fetch_subtitle_json(subtitle_url)
            if not subtitle_data:
                return ""

            # 4. 拼接为纯文本
            body = subtitle_data.get("body", [])
            text = " ".join(item.get("content", "") for item in body if item.get("content"))

            utils.logger.info(
                f"[BilibiliClient.get_video_subtitle] Got subtitle for bvid={bvid}, "
                f"lang={target.get('lan_doc', '')}, length={len(text)}"
            )
            return text

        except Exception as e:
            utils.logger.error(
                f"[BilibiliClient.get_video_subtitle] Failed to get subtitle for bvid={bvid}: {e}"
            )
            return ""

    async def _fetch_subtitle_json(self, subtitle_url: str) -> Dict:
        """
        下载字幕 JSON 文件

        Args:
            subtitle_url: 字幕文件完整 URL
        ...
        try:
            async with make_async_client(proxy=self.proxy) as client:
                response = await client.request(
                    "GET", subtitle_url, timeout=self.timeout, headers=self.headers
                )
                if response.status_code == 200:
                    return response.json()
                utils.logger.warning(
                    f"[BilibiliClient._fetch_subtitle_json] Status {response.status_code} for {subtitle_url}"
                )
                return {}
        except Exception as e:
            utils.logger.error(
                f"[BilibiliClient._fetch_subtitle_json] Failed to fetch subtitle: {e}"
            )
            return {}

    async def get_video_ai_summary(self, bvid: str, cid: int, aid: str = "") -> str:
        """
        通过 B 站 AI 总结 API 获取视频转录文本（降级方案）

        当视频没有 CC 字幕时，尝试使用 /x/web-interface/view/conclusion/get
        接口获取 AI 生成的逐句转录文本。
        ...
        """
        try:
            uri = "/x/web-interface/view/conclusion/get"
            params = {"bvid": bvid, "cid": cid}
            if aid:
                params["aid"] = aid
            ...
            model_result = resp.get("model_result", {})
            if not model_result:
                utils.logger.info(
                    f"[BilibiliClient.get_video_ai_summary] No AI summary for bvid={bvid}"
                )
                return ""

            # 优先提取逐句转录文本（part_subtitle）
            ai_subs_list = model_result.get("subtitle", [])
            if ai_subs_list:
                part_subs = ai_subs_list[0].get("part_subtitle", []) if ai_subs_list else []
                if part_subs:
                    text = " ".join(
                        item.get("content", "") for item in part_subs if item.get("content")
                    )
                    utils.logger.info(
                        f"[BilibiliClient.get_video_ai_summary] Got AI transcript for bvid={bvid}, "
                        f"segments={len(part_subs)}, length={len(text)}"
                    )
                    return text

            # 降级：使用 AI 摘要（summary 字段）
            summary = model_result.get("summary", "")
            if summary:
                utils.logger.info(
                    f"[BilibiliClient.get_video_ai_summary] Got AI summary for bvid={bvid}, "
                    f"length={len(summary)}"
                )
                return summary

            utils.logger.info(
                f"[BilibiliClient.get_video_ai_summary] No transcript or summary for bvid={bvid}"
            )
            return ""

        except Exception as e:
            utils.logger.warning(
                f"[BilibiliClient.get_video_ai_summary] Failed for bvid={bvid}: {e}"
            )
            return ""
