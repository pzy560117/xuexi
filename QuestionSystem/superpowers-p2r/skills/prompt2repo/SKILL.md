---
name: prompt2repo
description: "Prompt2Repo 一键入口: 从 prompt.md 到完整交付包的全自动流水线，串联 Phase 0-4.6 + 防假绿门禁 + 打包后三轮复部署测试门禁"
---
...
|:---|:---|:---|:---|
| **Micro** | FR ≤ 3 且无前端 | P0→P1→P2→P3→P3.9→P4→P4.5-4.6 | P0.5, P1.5, P2.5, P3.5-3.8, P4.1-4.4 |
| **Small** | FR 3-8 或 bug fix | P0→P0.5→P1→P2→P2.5→P3→P3.5→P3.9→P4→P4.5-4.6 | P1.5, P3.6-3.8, P4.1-4.4 |
...
本 Skill 是 Prompt2Repo 的**统一入口**，串联 Phase 0-4.6，实现从 Prompt 到可交付产物的一次执行全自动化，并在打包后执行三轮复部署测试门禁。
...
│                                                          │
│  Phase 4.6: Delivery Checker ★ 最终门禁                   │
│  ├─ 运行交付包最终验收                                     │
...
- Phase 4.6 最终验收失败 → 修复后自动复验，仍失败则输出阻塞项
