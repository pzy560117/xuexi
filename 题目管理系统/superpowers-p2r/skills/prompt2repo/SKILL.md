...
2. **主动管理阶段状态 (State Advancement Duty) ★ 绝对不可省略**：
   Loop 脚本无法自动猜测进度！当你准备结束当前阶段（即：准备输出该阶段完成承诺标签，如 `<promise>PROMPT_PARSING_COMPLETE</promise>` 等）时，你**必须**先使用文件写入工具（Write/Replace）主动修改 `.claude/superpower-loop.local.md` 文件：
...
执行 runtime-smoke skill，运行容器启动 + 健康检查 + API 冒烟测试。
...
执行 stability-loop skill，循环执行 runtime smoke（默认 3 轮）。
