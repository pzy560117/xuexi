---
name: prompt2repo
description: "Prompt2Repo 一键入口: 从 prompt.md 到完整交付包的全自动流水线，串联 Phase 0-4.7 + 防假绿门禁 + 打包后三轮复部署测试门禁"
---
...
> 灵感来源: [atelier-pipeline](https://github.com/robertsfeir/atelier-pipeline) Phase Sizing Rules

Phase 0 完成后，根据需求复杂度**自动判定流水线规模**，跳过不必要的 Phase：

| 规模 | 判断条件 | 执行 Phase | 跳过 Phase |
|:---|:---|:---|:---|
| **Micro** | FR ≤ 3 且无前端 | P0→P1→P2→P3→P3.9→P4→P4.5→P4.55→P4.6→P4.7 | P0.5, P1.5, P2.5, P3.5-3.8, P4.1-4.4 |
| **Small** | FR 3-8 或 bug fix | P0→P0.5→P1→P2→P2.5→P3→P3.5→P3.9→P4→P4.5→P4.55→P4.6→P4.7 | P1.5, P3.6-3.8, P4.1-4.4 |
| **Medium** | FR 8-20 | 全部执行，LLM 测试改为双轮 | P3.7, P4.3-4.4 |
| **Large** | FR 20+ 或全栈新系统 | 全部执行 + Agent Teams 并行 | 无跳过 |

**判定时机**：Phase 0 结束时写入 `docs/designs/_meta.md`：
```yaml
pipeline_size: "medium"   # micro | small | medium | large
fr_count: 12
skip_phases: ["consistency-gate"]
```

## 🔵 Wave 并行组 (Parallel Wave Groups)

> 灵感来源: atelier-pipeline Wave 波次模型 + Review Juncture 并行

以下 Phase **可并行执行**（无数据依赖）：

| Wave | 并行组成员 | 推进条件 |
|:---|:---|:---|
| Wave 3 | Phase 2.5 (Domain Checklist) ∥ Phase 3 (Self Review) | 全部完成 |
| Wave 4 (Large) | LLM R1 ∥ LLM R2 ∥ LLM R3（维度独立非递进） | 全部完成 |
| Wave 6 (Large) | Post-Pkg R1 ∥ Post-Pkg R2 ∥ Post-Pkg R3 | 全部完成 |

并行组状态跟踪于 `docs/runtime/parallel-group.json`。

## 概述

本 Skill 是 Prompt2Repo 的**统一入口**，串联 Phase 0-4.7，实现从 Prompt 到可交付产物的一次执行全自动化，并在打包后执行三轮复部署测试门禁。
...
说明：Phase 4.1 / 4.2 / 4.3 / 4.4（打包后复部署三轮测试 + 汇总门禁）为**强制阶段**，不提供 skip 参数。
...
│                                                          │
│  Phase 3.9: Test Truth Gate ★ 防假绿门禁                  │
│  ├─ 真实测试命令双跑（两轮退出码都必须为 0）                  │
│  ├─ 严查 run_tests.sh/.bat 是否存在吞错模式                  │
│  ├─ 强制产出 .tmp/test-truth-gate.md 证据                  │
│  └─ 未通过则回到当前阶段修复并重跑                           │
│  ✓ TEST_TRUTH_COMPLETE                                   │
│                                                          │
...
│  ✓ PACKAGE_COMPLETE                                      │
│                                                          │
│  Phase 4.1: Post Package Test Iteration 1 ★ 双轨门禁     │
│  ├─ 在 TASK 包上重新部署并执行 docs 要求测试                │
│  ├─ 主代理报告: .tmp/post-package-r1-main.md             │
│  └─ 子代理报告: .tmp/post-package-r1-subagent.md         │
│  ✓ POST_PACKAGE_TEST_R1_COMPLETE                         │
│                                                          │
│  Phase 4.2: Post Package Test Iteration 2 ★ 双轨门禁     │
│  ├─ 清理/重启后再次部署测试                                 │
│  ├─ 主代理报告: .tmp/post-package-r2-main.md             │
│  └─ 子代理报告: .tmp/post-package-r2-subagent.md         │
│  ✓ POST_PACKAGE_TEST_R2_COMPLETE                         │
│                                                          │
│  Phase 4.3: Post Package Test Iteration 3 ★ 双轨门禁     │
│  ├─ 边界与随机顺序复测                                     │
│  ├─ 主代理报告: .tmp/post-package-r3-main.md             │
│  └─ 子代理报告: .tmp/post-package-r3-subagent.md         │
│  ✓ POST_PACKAGE_TEST_R3_COMPLETE                         │
│                                                          │
│  Phase 4.4: Post Package Triple Check Gate ★ 一致性门禁  │
│  ├─ 汇总三轮 6 份报告                                      │
│  ├─ 校验 docs 要求覆盖与 PASS 一致性                        │
│  └─ 输出 .tmp/post-package-triple-check-gate.md          │
│  ✓ POST_PACKAGE_TRIPLE_CHECK_COMPLETE                    │
│                                                          │
│  Phase 4.5: Artifact Truth Gate ★ 真值门禁                │
│  ├─ 校验 TASK 包真实目录与报告一致性                        │
│  ├─ 拦截 PASS 但矛盾文本（Known Issue 等）                  │
│  └─ 输出 .tmp/artifact-truth-gate.md                     │
│  ✓ ARTIFACT_TRUTH_COMPLETE                               │
│                                                          │
│  Phase 4.55: Toolchain Validator Gate ★ 环境门禁         │
│  ├─ 强制校验 rg 可用性（含 fallback）                      │
│  ├─ 强制校验 validate_package.py 路径存在                 │
│  └─ 输出 .tmp/toolchain-validator-gate.md                │
│  ✓ TOOLCHAIN_VALIDATOR_COMPLETE                          │
│                                                          │
│  Phase 4.6: Delivery Checker ★ 证据门禁                  │
│  ├─ 运行交付包最终验收                                     │
│  ├─ 输出 docs/delivery-check-report.md                   │
│  ├─ 允许产出 FAIL>0 报告用于后续修复                        │
│  └─ 本阶段只负责“证据完整”，不负责最终放行                  │
│  ✓ DELIVERY_CHECK_COMPLETE                               │
│                                                          │
│  Phase 4.7: Release Readiness Gate ★ 最终放行门禁         │
│  ├─ 强制规则：FAIL=0 才允许放行                            │
│  ├─ 硬证据必须全为 0                                        │
│  └─ 输出 .tmp/release-readiness-gate.md                  │
│  ✓ DELIVERY_COMPLETE                                     │
│                                                          │
└─────────────────────────────────────────────────────────┘
```
...
### 12. Phase 3.9★ 防假绿门禁（强制）

```
执行 test-truth-gate skill。
要求：真实测试命令双跑、run_tests 脚本防假绿检查通过、产出 .tmp/test-truth-gate.md。
等待 `<promise>TEST_TRUTH_COMPLETE</promise>` 信号。
```
...
### 14. Phase 4.1★ 打包后复部署测试门禁

```
执行 post-package-test-iteration-1 skill，传入 --task-id 参数。
必须按 docs 验收要求完成重部署和测试，并输出主/子双报告。
等待 `<promise>POST_PACKAGE_TEST_R1_COMPLETE</promise>` 信号。
```

### 15. Phase 4.2★ 打包后复部署测试门禁

```
执行 post-package-test-iteration-2 skill，传入 --task-id 参数。
执行清理/重启后复测，并输出主/子双报告。
等待 `<promise>POST_PACKAGE_TEST_R2_COMPLETE</promise>` 信号。
```

### 16. Phase 4.3★ 打包后复部署测试门禁

```
执行 post-package-test-iteration-3 skill，传入 --task-id 参数。
执行边界/随机顺序复测，并输出主/子双报告。
等待 `<promise>POST_PACKAGE_TEST_R3_COMPLETE</promise>` 信号。
```

### 17. Phase 4.4★ 打包后三轮一致性门禁

```
执行 post-package-triple-check-gate skill，汇总三轮 6 份报告并校验 docs 要求覆盖。
等待 `<promise>POST_PACKAGE_TRIPLE_CHECK_COMPLETE</promise>` 信号。
```

### 18. Phase 4.5★ 产物真值门禁（强制）

```
执行 artifact-truth-gate skill，传入 --task-id 参数。
要求报告与真实文件系统一致（优先 repo/tests/unit_tests + repo/tests/API_tests；兼容 repo/unit_tests + repo/API_tests），且无矛盾文本。
等待 `<promise>ARTIFACT_TRUTH_COMPLETE</promise>` 信号。
```

### 19. Phase 4.55★ 工具链与校验器门禁（强制）

```
执行 toolchain-validator-gate skill，传入 --task-id 参数。
确保 rg 可用（或 fallback 可用）且 validate_package.py 路径就绪。
等待 `<promise>TOOLCHAIN_VALIDATOR_COMPLETE</promise>` 信号。
```

### 20. Phase 4.6★ 交付证据门禁（强制）

```
执行 delivery-checker skill，传入 --task-id 参数。
要求生成 `docs/delivery-check-report.md` 且硬证据字段完整。
等待 `<promise>DELIVERY_CHECK_COMPLETE</promise>` 信号。
```

### 21. Phase 4.7★ 最终放行门禁（强制）

```
执行 release-readiness-gate skill，传入 --task-id 参数。
强制规则：FAIL=0 且关键硬证据为 0。
...
phases:
  ...
  - name: "test-truth-gate"
    status: "pending"
    completion_promise: "TEST_TRUTH_COMPLETE"
    skippable: false
    prompt: "强制防假绿：真实测试命令双跑必须都为 0，且 run_tests.sh/.bat 通过脚本 lint（不得吞错），输出 .tmp/test-truth-gate.md。"
  ...
  - name: "post-package-test-iteration-1"
    status: "pending"
    completion_promise: "POST_PACKAGE_TEST_R1_COMPLETE"
    skippable: false
    prompt: "在 TASK 包上执行第 1 轮重部署+测试，必须按 docs 验收项覆盖并产出 .tmp/post-package-r1-main.md 与 .tmp/post-package-r1-subagent.md（双 PASS）。"
  - name: "post-package-test-iteration-2"
    status: "pending"
    completion_promise: "POST_PACKAGE_TEST_R2_COMPLETE"
    skippable: false
    prompt: "在清理/重启后执行第 2 轮重部署+测试，必须产出 r2 双报告且双 PASS。"
  - name: "post-package-test-iteration-3"
    status: "pending"
    completion_promise: "POST_PACKAGE_TEST_R3_COMPLETE"
    skippable: false
    prompt: "执行第 3 轮边界/随机顺序重部署+测试，必须产出 r3 双报告且双 PASS。"
  - name: "post-package-triple-check-gate"
    status: "pending"
    completion_promise: "POST_PACKAGE_TRIPLE_CHECK_COMPLETE"
    skippable: false
    prompt: "汇总 post-package 三轮 6 份报告，校验 docs 要求覆盖与一致性，输出 .tmp/post-package-triple-check-gate.md。"
  - name: "artifact-truth-gate"
    status: "pending"
    completion_promise: "ARTIFACT_TRUTH_COMPLETE"
    skippable: false
    prompt: "强制校验 post-package 报告与 TASK 包真实目录一致（优先 repo/tests/unit_tests 与 repo/tests/API_tests；兼容 repo/unit_tests 与 repo/API_tests），并拦截 PASS 但矛盾文本，输出 .tmp/artifact-truth-gate.md。"
  - name: "toolchain-validator-gate"
    status: "pending"
    completion_promise: "TOOLCHAIN_VALIDATOR_COMPLETE"
    skippable: false
    prompt: "强制校验工具链与验收脚本路径：rg 可用（或 fallback 可用）、validate_package.py 路径存在，并输出 .tmp/toolchain-validator-gate.md。"
  - name: "delivery-checker"
    status: "pending"
    completion_promise: "DELIVERY_CHECK_COMPLETE"
    skippable: false
    prompt: "执行最终交付验收脚本，确保 docs/delivery-check-report.md 证据字段完整，输出 DELIVERY_CHECK_COMPLETE。"
  - name: "release-readiness-gate"
    status: "pending"
    completion_promise: "DELIVERY_COMPLETE"
    skippable: false
    prompt: "最终放行门禁：强制 FAIL=0 + 核心硬证据全 0，并输出 .tmp/release-readiness-gate.md。"
---
```
...
- Phase 3.9 防假绿门禁失败（测试双跑不一致、run_tests 吞错等）→ 必须修复并重跑当前门禁，禁止进入打包
- Phase 4 validate 失败 → 执行 `--repair`，仍失败则输出错误
- Phase 4.1/4.2/4.3 任一轮打包后复部署测试 FAIL → 必须在当前轮自动修复并重跑直到主/子双 PASS
- Phase 4.4 打包后三轮一致性门禁失败 → 返回对应轮次补测并重新汇总，不得跳过
- Phase 4.5 产物真值门禁失败 → 必须回到对应 post-package 轮次修复并重跑，再次汇总后复验
- Phase 4.55 工具链门禁失败（rg/validate_package.py 不可用）→ 必须先在当前阶段自动修复，再继续
- Phase 4.6 交付证据门禁失败（报告缺失或证据字段不完整）→ 修复后自动复验
- Phase 4.7 最终放行门禁失败（FAIL>0）→ 必须回到 4.5/4.55/4.6 对应阶段修复并重跑，禁止输出 DELIVERY_COMPLETE
...
/superpowers:test-truth-gate                                      # Phase 3.9 ★
/superpowers:delivery-packager --task-id TASK-20260327-XXXXX      # Phase 4
/superpowers:post-package-test-iteration-1 --task-id TASK-20260327-XXXXX   # Phase 4.1 ★
/superpowers:post-package-test-iteration-2 --task-id TASK-20260327-XXXXX   # Phase 4.2 ★
/superpowers:post-package-test-iteration-3 --task-id TASK-20260327-XXXXX   # Phase 4.3 ★
/superpowers:post-package-triple-check-gate --task-id TASK-20260327-XXXXX  # Phase 4.4 ★
/superpowers:artifact-truth-gate --task-id TASK-20260327-XXXXX     # Phase 4.5 ★
/superpowers:toolchain-validator-gate --task-id TASK-20260327-XXXXX # Phase 4.55 ★
/superpowers:delivery-checker --task-id TASK-20260327-XXXXX       # Phase 4.6 ★
/superpowers:release-readiness-gate --task-id TASK-20260327-XXXXX # Phase 4.7 ★
```
